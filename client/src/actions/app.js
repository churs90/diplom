...
export const signUp = ({username, password}) => {
	...
	return dispatch => {
		...
		API.post('auth/sign-up', { username, password })
			.then(res => {
				if(res.code == 200){
					cogoToast.success(`Добро пожаловать @${res.response.username}!`, {
...
export const signIn = ({username, password}) => {
	return dispatch => {
		...
		API.post('auth/sign-in', { username, password })
			.then(res => {
				if(res.code == 200){
					cogoToast.success(`С возвращением @${res.response.username} :)!`, {
					    position: 'bottom-right'
					});
					dispatch({
						type: SIGN_IN,
...
export const setProfilePic = url => {
	return dispatch => {
		cogoToast.success(`Изобраление загружено!`, {
		    position: 'bottom-right'
		});
		dispatch({
...
export const setDescription = description => {
	return dispatch => {
		cogoToast.success(`Статус изменён!`, {
		    position: 'bottom-right'
		});
		dispatch({
